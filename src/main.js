...
const getDuration = (file) => {
    ...
    let duration = JSON.parse(execSync(ffprobe + " -i " + file + " -print_format json -show_format -select_streams v -hide_banner").toString()).format.duration;
    return (duration);
}
...
const ffmpegConfig = {
    mp4: {
        Ultra: ["-preset", "veryslow", "-codec:a", "aac", "-b:a", "320k", "-codec:v", "libx264", "-crf", "15", "-pix_fmt", "yuv420p", "-movflags", "+faststart"],

        High: ["-preset", "slow", "-codec:a", "aac", "-b:a", "256k", "-codec:v", "libx264", "-crf", "17", "-pix_fmt", "yuv420p", "-movflags", "+faststart"],

        Medium: ["-preset", "fast", "-codec:a", "aac", "-b:a", "192k", "-codec:v", "libx264", "-crf", "22", "-pix_fmt", "yuv420p", "-movflags", "+faststart"],

        Low: ["-preset", "veryfast", "-codec:a", "aac", "-b:a", "128k", "-codec:v", "libx264", "-crf", "26", "-pix_fmt", "yuv420p", "-movflags", "+faststart"]
    },
    webm: {
        Ultra: ["-codec:a", "libopus", "-b:a", "320k", "-codec:v", "libvpx-vp9", "-crf", "25", "-b:v", "0", "-pix_fmt", "yuv420p"],

        High: ["-codec:a", "libopus", "-b:a", "256k", "-codec:v", "libvpx-vp9", "-crf", "27", "-b:v", "0", "-pix_fmt", "yuv420p"],

        Medium: ["-codec:a", "libopus", "-b:a", "192k", "-codec:v", "libvpx-vp9", "-crf", "32", "-b:v", "0", "-pix_fmt", "yuv420p"],

        Low: ["-codec:a", "libopus", "-b:a", "128k", "-codec:v", "libvpx-vp9", "-crf", "36", "-b:v", "0", "-pix_fmt", "yuv420p"]
    }
}
...
const process = async(data, callback) => {
    ...
    if (data.numberOfVideos == 1) {
        task = ["-y", "-ss", data.startTime, "-i", data.video];
        ...
        duration = getDuration(data.video);
